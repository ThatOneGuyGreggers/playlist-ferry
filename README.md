...
Version 2.0.2 is a maintenance release published to exercise the in-app update path introduced in 2.0.1. It includes YouTube video and playlist artwork, full resolution of YouTube playlist links, corrected Apple Music playlist paths, direct YouTube sources, Apple Music-compatible playlist exports, configurable concurrent downloads, and per-track manual YouTube repair. Release bundles include Python, FFmpeg, FFprobe, and Deno and are tested on Intel and Apple Silicon. They are ad hoc signed, so macOS may require manual approval in Privacy & Security.
